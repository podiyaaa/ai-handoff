...
inadvertently published to the VS Code Marketplace with the pre-release flag
set and never re-published as stable — since a version number can't be
re-uploaded, non-pre-release Marketplace installs were stuck offering `0.2.0`.
This release exists solely to give Marketplace users a current, correctly
stable-flagged version to install.
