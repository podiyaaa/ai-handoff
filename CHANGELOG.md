...
- **Ticking one file's checkbox could select its entire parent folder** — the real bug, found via diagnostic logging after two wrong guesses. VS Code's native tree checkbox feature auto-includes every ancestor directory in the same checkbox-change event, marking them "checked" too, to keep ancestor checkboxes visually in sync — this happens on every checkbox click, with or without search involved. `handleCheckboxChange` was treating every entry in that batch as an independent user action, so those auto-included ancestors triggered full recursive bulk-selects nobody asked for (worse the deeper the file — checking a file three folders deep bulk-selected all three ancestor folders). Fixed by only acting on the one item in a batch that isn't an ancestor of any other item in the same batch — everything else is VS Code's sync notification, not a click, and is now ignored. Two earlier, incorrect fixes (an `id`-churn scheme, then automatic `TreeView.reveal()` calls on search) had wrongly blamed search-triggered auto-expand for this and consequently disabled it; both are reinstated now that the actual cause is fixed at the source, backed by a new `FileTreeProvider.getParent()`.
