...
- **Search/filter bar above the sidebar file tree** — a persistent search box (its own small panel, pinned directly above the Files tree) filters the tree down to matching files as you type. Supports three modes: plain text (substring match against the path), `ext:ts,tsx` (match by extension), and `re:^use[A-Z]` (regex, case-insensitive), with an inline error hint for invalid input. Directories being rendered for the first time while a search is active start expanded so matches are reachable, and a clear (✕) button appears while the box has text. Purely a display filter — it never changes what's selected.
...
- **Checkbox clicks near a search change could select the wrong item** — search-triggered auto-expand forced already-rendered folders open, first by tagging every tree item's `id` with a counter bumped on every search keystroke, later by calling `TreeView.reveal()` on every matching directory each time a search settled. Both approaches reshape the tree's visible rows as an automatic side effect of typing, and both were confirmed (by reproducing it twice) to occasionally misroute an in-flight checkbox click to the wrong (ancestor) node — e.g. ticking a second file found via a second search could end up toggling its entire parent folder instead, silently selecting many unintended files. Search-triggered expansion is now purely the *default* `collapsibleState` for a directory being rendered for the first time — no forced re-expansion of already-rendered nodes, and no automatic `reveal()` calls at all. "Expand All" (a single deliberate button click, not something driven by every keystroke) is the reliable way to force everything open, backed by `TreeView.reveal()` + a new `FileTreeProvider.getParent()`, alongside VS Code's native "Collapse All".
